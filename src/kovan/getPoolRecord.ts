...
async function getPoolContract(
  ...
  let poolToken = await getFarmingV2Contract(farm.address, provider).lpToken(adjusterPoolId);
...
export async function getPoolRecord(
  ...
  try {
    ...
    const provider = await getProvider(chainId);
    const farming_V2 = getFarmingV2Contract(getAddress(AddressName.FARMING_V2, chainId), provider) as FarmingContracts;
    ...
    let poolToken = await getFarmingV2Contract(farm.address, provider).lpToken(adjustedPoolId);
    ...
    if (isFarmGeneric(poolId)) {
      let res = await getGenericFarmingV2Contract(farm.address, provider).rewardTokensPerBlock();
      ...
      let ichiPerBlock_V2 = await getFarmingV2Contract(farm.address, provider).ichiPerBlock();
      rewardsPerBlock = Number(ichiPerBlock_V2);
    }

    let totalAllocPoint = Number(await farm.totalAllocPoint());
    let poolInfo = await getFarmingV2Contract(farm.address, provider).poolInfo(adjustedPoolId);
    ...
    const poolContract = await getPoolContract(poolId, farm, adjustedPoolId, chainId);
    ...
    console.log(`Error calling getPoolRecord`, e);
    throw e;
  }
}
